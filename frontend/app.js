// Auto-generated from frontend/app.ts. Do not edit this file directly.
...
const isLocalHostname = [
    ...
    "localhost",
    "0.0.0.0"
].includes(location.hostname) || location.hostname.endsWith(".local");
const isLocal = isLocalHostname || isLanIp;
const API_BASE = isLocal ? `http://127.0.0.1:54321/functions/v1/${FUNCTION_NAME}` : `https://${PROJECT_REF}.functions.supabase.co/${FUNCTION_NAME}`;
...
function translateValidationMessage(msg) {
    ...
    const map = [
        {
            k: "scores must have 4 items",
            v: "スコアは4人分すべて入力してください。"
        },
        {
            k: "invalid score item",
            v: "スコアの形式が不正です。もう一度入力してください。"
        },
        {
            k: "seat_index must be 0..3",
            v: "内部エラー: 座席番号が不正です。ページを再読み込みしてやり直してください。"
        },
        {
            k: "raw_score must be multiple of 100",
            v: "各スコアは100点単位で入力してください。"
        },
        {
            k: "each seat_index must appear once",
            v: "4人分それぞれ1回ずつ入力してください。"
        },
        {
            k: "scores must include seat_index 0..3",
            v: "4人分のスコアをすべて入力してください。"
        },
        {
            k: "sum must be 100000",
            v: "合計は100000点です。入力値を確認してください。"
        },
        {
            k: "game number must be a positive integer",
            v: "対象ゲーム番号が不正です。画面を再読み込みしてやり直してください。"
        },
        {
            k: "同時更新が発生しました",
            v: "他のユーザー更新と競合しました。再読み込みしてもう一度お試しください。"
        },
        {
            k: "not found",
            v: "対局が見つかりません。"
        ...
        {
            k: "oka_points / uma_low / uma_high は整数で入力してください",
            v: "オカ・ウマは整数で入力してください。"
        },
        {
            k: "ウマは自然数で入力してください",
            v: "ウマは自然数（例: 5-10 のように）で入力してください。"
        },
        {
            k: "ウマは high >= low となるように入力してください",
            v: "ウマは「low-high」で high が low 以上になるように入力してください。"
        }
    ];
...
function route() {
    const info = typeof resolveRouteView === "function" ? resolveRouteView(location.hash || "", location.pathname) : (()=>{
...
async function createMatch() {
    ...
    try {
        ...
        $("oka_select").value = "30000";
        $("oka_custom").value = "";
        $("oka_custom_wrap").style.display = "none";
        $("uma_select").value = "5-15";
        $("uma_custom").value = "";
...
function renderMatches(list) {
    ...
    list.forEach((m)=>{
        ...
        pill1.textContent = `オカ ${m.oka_points}`;
        const pill2 = document.createElement("span");
        pill2.className = "pill";
        pill2.textContent = `ウマ ${Math.abs(m.uma_low)}/${Math.abs(m.uma_high)}`;
        const pill3 = document.createElement("span");
        pill3.className = "pill";
        pill3.textContent = `対局数 ${gameCount}`;
        const pill4 = document.createElement("span");
        pill4.className = "pill";
        pill4.textContent = `更新 ${when.toLocaleString("ja-JP")}`;
        meta.append(pill1, pill2, pill3, pill4);
...
function renderDetail(m) {
    ...
    const defaults = [
        ...
        2,
        3
    ].map((i)=>currentParticipants.find((p)=>p.seat_priority === i)?.name || "");
...
function enforceUniqueSeatNames() {
    ...
    Array.from(sels[0].options).forEach((opt)=>{
        opt.disabled = false;
    });
    ...
    if (remain.length === 1) {
        Array.from(sels[3].options).forEach((opt)=>{
            opt.disabled = opt.value !== remain[0];
        });
    } else {
        Array.from(sels[3].options).forEach((opt)=>{
            opt.disabled = false;
...
function renderGames(m) {
    ...
    if (!games.length) {
        ...
        const td = document.createElement("td");
        td.colSpan = 4;
        td.textContent = "ゲームはまだ登録されていません。上のフォームから登録しましょう。";
        tr.appendChild(td);
    ...
    const fallbackNames = [
        ...
        2,
        3
    ].map((i)=>currentParticipants.find((p)=>p.seat_priority === i)?.name || `Seat ${i}`);
    const participantNameById = new Map((currentParticipants || []).map((p)=>[
            String(p.id),
            p.name
        ]));
    rows.forEach((r)=>{
        ...
        inp.value = String(r.raw_score);
        inp.id = `editScore-${r.seat_index}`;
        tdScore.appendChild(inp);
        const tdTotal = document.createElement("td");
        tdTotal.textContent = r.pt_total.toFixed(1);
        ...
        if (cardList) {
            ...
            rankBadge.textContent = String(r.rank);
            ...
            totalEl.textContent = r.pt_total.toFixed(1) + "pt";
            totalEl.className = r.pt_total > 0 ? "pos" : r.pt_total < 0 ? "neg" : "";
            rowTop.appendChild(left);
            rowTop.appendChild(totalEl);
            const rowBottom = document.createElement("div");
            const inp2 = document.createElement("input");
            inp2.type = "text";
            inp2.setAttribute('inputmode', 'numeric');
            inp2.setAttribute('pattern', '[0-9\\-]*');
            inp2.className = 'score-input';
            inp2.value = String(r.raw_score);
            inp2.id = `editScoreCard-${r.seat_index}`;
...
function renderTotals(m) {
    ...
    totalsSrv.forEach((t)=>{
        if (t && typeof t.participant_id === 'string') byPid.set(t.participant_id, t.total || 0);
    });
    if (byPid.size === 0) {
        const games = Array.isArray(m.games) ? m.games : [];
        games.forEach((g)=>{
    ...
    rows.forEach((r, i)=>{
        ...
        const tdTotal = document.createElement("td");
        tdTotal.textContent = `${r.total.toFixed(1)}pt`;
...
async function submitScores() {
    ...
    if (!id) return;
    const namesBySeat = [
        $("seat0name").value.trim(),
        $("seat1name").value.trim(),
        $("seat2name").value.trim(),
        $("seat3name").value.trim()
    ];
    const scores = [
        0,
        1,
        2,
        3
    ].map((i)=>Number($(`seat${i}`).value));
    const nameToId = new Map((currentParticipants || []).map((p)=>[
            String(p.name),
            String(p.id)
        ]));
    const players = [
        ...
        return {
            seat_index: i,
            participant_id: pid,
            display_name: nm || undefined,
            name: nm || undefined
        };
    });
    if (players.some((p)=>!p.participant_id)) {
        showScoreError("参加者の選択が不正です");
        return;
    }
    if (scores.some((v)=>!Number.isInteger(v))) {
        showScoreError("整数で入力してください");
        return;
    }
    if (scores.some((v)=>v % 100 !== 0)) {
        showScoreError("raw_score は100点単位");
        return;
    }
    const sum = scores.reduce((a, b)=>a + b, 0);
    if (sum !== 100000) {
        const delta = 100000 - sum;
...
function drawTrendChartFromGames(m) {
    ...
    const seatIndexToOrderedIdx = new Map([
        0,
        1,
        2,
        3
            ...
            i,
            ordered.findIndex((p)=>p.seat_priority === i)
        ]));
    games.forEach((g)=>{
        (g.results || []).forEach((r)=>{
            const pid = r.participant_id;
    ...
    for(let i = 0; i <= steps; i++){
        ...
        ctx.fillText(yy.toFixed(1), 4, ypx + 4);
    }
    (games || []).forEach((g)=>{
        const xpx = X(g.number);
        ctx.beginPath();
        ctx.moveTo(xpx, H - margin.bottom);
        ctx.lineTo(xpx, H - margin.bottom + 4);
        ctx.strokeStyle = "#9ca3af";
        ctx.stroke();
        ctx.fillStyle = "#6b7280";
        ctx.fillText(String(g.number), xpx - 6, H - margin.bottom + 16);
    });
    ctx.strokeStyle = "#9ca3af";
    ctx.strokeRect(margin.left, margin.top, W - margin.left - margin.right, H - margin.top - margin.bottom);
    const hoverDot = window.__trendLocked || window.__trendHover || null;
    ...
    if (hoverDot) {
        ...
        if (p) {
            ...
            let w = Math.ceil(metrics.width) + padX * 2;
            if (w < 80) w = 80;
            const h = 14 + padY * 2;
            ...
            if (y0 < margin.top) {
                y0 = Math.round(py + arrowH + gap);
                above = false;
                if (y0 + h > H - margin.bottom) y0 = H - margin.bottom - h;
            }
            ctx.save();
...
function bindTrendPointerOnce(canvas) {
    ...
    function selectFromPointCoords(clientX, clientY) {
        const geom = window.__trendGeom;
        const games = window.__trendGames || [];
    ...
    const onMove = (ev)=>{
        ...
        const within = bestDot && Math.sqrt(best) <= 20;
        if (within) {
            window.__trendHover = {
                i: bestDot.i,
                game: bestDot.game
            ...
            drawTrendChartFromGames({
                games: window.__lastGames || []
            });
            if (tip) tip.style.display = 'none';
            return;
            const names = window.__trendNames || [];
            tip.innerHTML = `<div style="font-weight:600;margin-bottom:4px;">Game ${bestDot.game}</div>` + `<div><span style="display:inline-block;width:10px;height:3px;background:${TREND_COLORS[bestDot.i % TREND_COLORS.length]};margin-right:6px;"></span>` + `${names[bestDot.i] || 'Seat ' + bestDot.i}: ${bestDot.y.toFixed(1)}pt</div>`;
    ...
    const onClick = (ev)=>{
        ...
        if (sel.type === 'dot' || sel.type === 'snap') {
            window.__trendLocked = {
                i: sel.bestDot.i,
                game: sel.bestDot.game
            };
            window.__trendHighlight = sel.bestDot.game;
        } else {
            window.__trendLocked = null;
            window.__trendHover = null;
            window.__trendHighlight = null;
        }
        drawTrendChartFromGames({
...
window.addEventListener("DOMContentLoaded", ()=>{
    ...
    $("oka_select").addEventListener("change", ()=>{
        $("oka_custom_wrap").style.display = $("oka_select").value === "custom" ? "block" : "none";
    });
    $("uma_select").addEventListener("change", ()=>{
        $("uma_custom_wrap").style.display = $("uma_select").value === "custom" ? "block" : "none";
    });
    window.addEventListener("resize", ()=>{
        if (currentMatchId) {
            const canvas = $("trendCanvas");
            if (canvas) drawTrendChartFromGames({
                games: window.__lastGames || []
            });
        }
    });
